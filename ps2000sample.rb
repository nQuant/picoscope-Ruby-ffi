...
def collect_block_immediate()
	...
	while (ps2000_get_timebase($unitOpened[:handle], timebase, no_of_samples, time_interval_ptr, time_units_ptr, oversample, max_samples_ptr)!=0)
	end
	...
	ps2000_get_times_and_values($unitOpened[:handle], $times_ptr, $unitOpened[:channelSettings][PS2000_CHANNEL_A][:values], $unitOpened[:channelSettings][PS2000_CHANNEL_B][:values], nil, nil, overflow_ptr, time_units_ptr.read_int16, no_of_samples)
	
	print( "First 10 readings\n\n" );
	print( "Time(%s) Values\n" % adc_units ( time_units_ptr.read(:int16) ) );
	for i in (0..9)
...
def get_info()
	...
	if $unitOpened[:handle]
		...
		ps2000_get_unit_info ( $unitOpened[:handle], FFI::MemoryPointer.from_string(line), line.length, 5 );
...
def main()
	...
	if $unitOpened[:handle]!=0
		...
		get_info
		puts "Appuyez sur <ENTREE> pour poursuivre\n"
		STDIN.gets
